...
(async () => {
  try {
    ...
    console.log(`-> 获取到第1页，中国用户共${data.items.length}条数据！`);
    await sleep(1000);

    data = await getUserData(2, true) as IResultUserData;
    users = users.concat(data.items);
    console.log(`-> 获取到第2页，中国用户共${data.items.length}条数据！`);
    await sleep(1000);

    data = await getUserData(3, true) as IResultUserData;
    users = users.concat(data.items);
    console.log(`-> 获取到第3页，中国用户共${data.items.length}条数据！`);
    await sleep(1000);

    data = await getUserData(4, true) as IResultUserData;
    users = users.concat(data.items);
    console.log(`-> 获取到第4页，中国用户共${data.items.length}条数据！`);
    await sleep(1000);

    data = await getUserData(5, true) as IResultUserData;
    users = users.concat(data.items);
    console.log(`-> 获取到第5页，中国用户共${data.items.length}条数据！`);
