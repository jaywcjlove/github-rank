...
;(async () => {
  try {
    // 获取【全球】用户数据排行榜
    const users: IUserData[] = await saveUsersData(usersDist, usersCache, '');
    ...
    // 获取【中国】用户数据排行榜
    const usersChina: IUserData[] = await saveUsersData(usersChinaDist, usersChinaCache, '.china', users);
